    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <header className="flex justify-between items-end">
        ...
        <div className="flex gap-4">
          ...
          >
            {searchingMaps ? <Loader2 className="w-4 h-4 animate-spin" aria-hidden="true" /> : <MapPin className="w-4 h-4 text-emerald-400" aria-hidden="true" />}
            Scan Nearby Support
          </button>
          <button 
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl" aria-label="Inventory Table and Filters">
        <div className="p-6 border-b border-slate-800 flex flex-col md:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
             <button 
                ...
                className={`p-3 rounded-xl border transition-all ${showFilters ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'bg-slate-950 border-slate-800 text-slate-400'}`}
             >
                <Filter className="w-4 h-4" aria-hidden="true" />
             </button>
             <div className="h-10 w-[1px] bg-slate-800" aria-hidden="true"></div>
        ...
        {showFilters && (
          <div className="p-6 bg-slate-950/50 border-b border-slate-800 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-top-4">
             <div className="space-y-2">
                <label htmlFor="vendor-filter" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Vendor Filter</label>
                <select 
                  id="vendor-filter"
             ...
             <div className="space-y-2">
                <label htmlFor="role-filter" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Operational Role</label>
                <select 
                  id="role-filter"
                  value={selectedRole}
             ...
             <div className="space-y-2">
                <label htmlFor="status-filter" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Health Status</label>
                <select 
                  id="status-filter"
        ...
        <div className="overflow-x-auto">
                ...
                >
                  <td className="px-8 py-4">
                    <div className="flex items-center gap-4">
                      <div className={`p-2 rounded-xl border ${device.status === 'Alerting' ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' : 'bg-slate-800 border-slate-700 text-slate-400 group-hover:text-emerald-400'}`} aria-hidden="true">
                  ...
                  <td className="px-8 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-24 h-1.5 bg-slate-950 rounded-full overflow-hidden" role="progressbar" aria-valuenow={device.healthScore} aria-valuemin={0} aria-valuemax={100}>
                        <div 
                  ...
                  <td className="px-8 py-4 text-right">
                    <button className="p-2 text-slate-600 hover:text-white transition-all" aria-label="More actions"><MoreVertical className="w-4 h-4" /></button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {filteredDevices.length === 0 && (
            <div className="p-20 text-center opacity-40" aria-live="polite">
               <Database className="w-12 h-12 mx-auto mb-4 text-slate-700" aria-hidden="true" />
      ...
      {mapsResult && (
        <section className="bg-slate-950 border border-slate-800 rounded-3xl p-8 animate-in slide-in-from-bottom-8" aria-labelledby="maps-result-heading">
           <div className="flex justify-between items-center mb-6">
              <h3 id="maps-result-heading" className="text-white font-bold flex items-center gap-2">
                <MapPin className="w-5 h-5 text-emerald-400" aria-hidden="true" /> Nearby Support Infrastructure
              </h3>
              <button onClick={() => setMapsResult(null)} aria-label="Close maps results" className="text-slate-500 hover:text-white"><X className="w-5 h-5" /></button>
           </div>
              ...
              {mapsResult.chunks?.map((chunk: any, i: number) => chunk.maps && (
                <a key={i} href={chunk.maps.uri} target="_blank" rel="noopener noreferrer" className="bg-slate-900 p-4 rounded-2xl border border-slate-800 hover:border-emerald-500/30 transition-all flex flex-col gap-2 group">
                   <div className="flex justify-between items-start">
                      ...
                      <ExternalLink className="w-3 h-3 text-slate-600 group-hover:text-emerald-400" aria-hidden="true" />
                   </div>
      ...
      {/* Add Device Modal */}
      {isAddModalOpen && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300" role="dialog" aria-modal="true" aria-labelledby="add-node-heading">
          <div className="bg-slate-900 border border-slate-800 rounded-[32px] w-full max-w-xl p-10 shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 to-blue-500" aria-hidden="true"></div>
            <div className="flex justify-between items-center mb-8">
               <div className="flex items-center gap-3">
                 <div className="bg-emerald-500/10 p-2.5 rounded-xl border border-emerald-500/20" aria-hidden="true">
                    <PlusCircle className="w-6 h-6 text-emerald-400" />
                 </div>
                 <div>
                    <h3 id="add-node-heading" className="text-xl font-bold text-white tracking-tight">Onboard New Node</h3>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Ansible Orchestration Required</p>
                 </div>
               </div>
               <button onClick={() => setIsAddModalOpen(false)} aria-label="Close modal" className="p-2 text-slate-500 hover:text-white transition-all"><X className="w-6 h-6" /></button>
            </div>

            <form onSubmit={handleAddDevice} className="space-y-6">
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label htmlFor="new-device-name" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Hostname / Label</label>
                    <input 
                      id="new-device-name"
                      required
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label htmlFor="new-device-ip" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">IP Address</label>
                    <input 
                      id="new-device-ip"
                      required
                      type="text" 
               ...
               <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label htmlFor="new-device-vendor" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Vendor Platform</label>
                    <select 
                      id="new-device-vendor"
                  ...
                  <div className="space-y-2">
                    <label htmlFor="new-device-role" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Deployment Role</label>
                    <select 
                      id="new-device-role"
               ...
               <div className="space-y-2">
                  <label htmlFor="new-device-model" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest px-1">Hardware Model / OS Version</label>
                  <input 
                    id="new-device-model"
                    type="text" 
               ...
               <div className="pt-6 border-t border-slate-800 flex gap-4">
                  ...
                  >
                    {isProvisioning ? <Loader2 className="w-5 h-5 animate-spin" aria-hidden="true" /> : <Zap className="w-5 h-5" aria-hidden="true" />}
      ...
      {selectedDevice && (
        <div className="fixed inset-0 z-[160] flex items-center justify-end animate-in fade-in duration-300" role="dialog" aria-modal="true" aria-labelledby="device-details-heading">
           <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" onClick={() => setSelectedDevice(null)} aria-hidden="true"></div>
           <div className="w-full max-w-2xl h-full bg-slate-900 border-l border-slate-800 shadow-2xl relative animate-in slide-in-from-right-full duration-500 flex flex-col">
              <div className="p-8 border-b border-slate-800 bg-slate-950/20">
                 <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-4">
                       <div className="p-4 bg-emerald-500/10 rounded-2xl border border-emerald-500/20" aria-hidden="true">
                          <Cpu className="w-8 h-8 text-emerald-400" />
                       </div>
                       <div>
                          <h3 id="device-details-heading" className="text-2xl font-black text-white tracking-tight">{selectedDevice.name}</h3>
                          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">{selectedDevice.vendor} • {selectedDevice.model}</p>
                       </div>
                    </div>
                    <button onClick={() => setSelectedDevice(null)} aria-label="Close details" className="p-3 bg-slate-800 hover:bg-slate-700 rounded-xl text-slate-400 transition-all"><X className="w-5 h-5" /></button>
                 </div>
              ...
              <div className="flex border-b border-slate-800 px-8" role="tablist">
                 {[
              ...
              <div className="flex-1 p-8 overflow-y-auto bg-slate-950/10 custom-scrollbar">
                 <div id={`tab-panel-${activeDetailTab}`} role="tabpanel" tabIndex={0} className="outline-none">
                      ...
                      <div className="space-y-8 animate-in fade-in duration-300">
                          ...
                          <div className="bg-slate-900/50 p-8 rounded-[32px] border border-slate-800">
                            <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
                                <Gauge className="w-4 h-4 text-emerald-400" aria-hidden="true" /> Throughput (Gbps)
                            </h4>
                            <div className="h-40 flex items-end gap-1.5 px-2" aria-hidden="true">
                                {[...Array(24)].map((_, i) => (
                    ...
                    {activeDetailTab === 'ztp' && (
                      <div className="space-y-4 animate-in fade-in duration-300" aria-live="polite">
                          {[
