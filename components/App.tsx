    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <div className="bg-slate-900 border border-slate-800 p-12 rounded-3xl max-w-md w-full text-center space-y-8 shadow-2xl relative overflow-hidden">
          ...
          <button onClick={handleKeyActivation} className="w-full py-5 bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-bold rounded-2xl shadow-xl shadow-emerald-500/20 uppercase tracking-widest text-xs">Verify System Key</button>
        </div>
      </div>
    );
  }
          ...
          <div className={`flex items-center gap-6 ${isUrdu ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`flex items-center gap-4 group ${isUrdu ? 'flex-row-reverse' : ''}`}>
              ...
              <div className="relative">
                ...
                <button onClick={handleLogout} className="absolute -bottom-1 -right-1 p-1 bg-slate-900 border border-slate-800 rounded-lg text-slate-500 hover:text-red-400 transition-all"><LogOut className="w-3 h-3" /></button>
              </div>
            </div>
          </div>
        </div>
